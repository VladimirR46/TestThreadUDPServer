...
public:
	...
	auto operator()(const boost::system::error_code& ec, std::size_t bytes, Callback callback) const
	{
		...
		return callback(sender_endpoint, ec, bytes, buffer_begin, buffer_begin + std::min((std::size_t)std::max(0l, std::distance(buffer_begin, buffer_end)), bytes));
	}
};
